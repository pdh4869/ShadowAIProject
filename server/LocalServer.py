...
from Logic import handle_input_raw, detect_by_ner, detect_by_regex, mask_pii_in_filename
                ...
                try {
                    ...
                    if (data.detections && data.detections.length > 0) {
                        logContainer.innerHTML = data.detections.map(d => {
                            const statusHtml = d.status === 'valid' ? `<span class="status-valid">(valid)</span>` : (d.status ? `<span class="status-invalid">(${escapeHtml(d.status)})</span>` : '');
                            ...
                            if (d.type === 'group' && d.items) {
                                ...
                                <div class="log-entry">
                                    ...
                                    ${d.items.map(item => {
                                        const itemStatusHtml = item.status === 'valid' ? `<span class="status-valid">(valid)</span>` : (item.status ? `<span class="status-invalid">(${escapeHtml(item.status)})</span>` : '');
                                        return `
...
async def handle_file_collect(request: Request):
    ...
    try:
        ...
        original_file_name = data.get("name", "unknown")
        ...
        masked_name, pii_type = mask_pii_in_filename(original_file_name)
        display_name = f"{masked_name} ({pii_type})" if pii_type else original_file_name

        if pii_type:
            print(f"[INFO] 파일명 마스킹 적용: {display_name}")
        ...
        if estimated_size > SOFT_LIMIT:
            print(f"[WARN] 큰 파일 처리 중: {estimated_size / 1024 / 1024:.1f}MB - {original_file_name}")
        
        print(f"[INFO] 파일 수신: {original_file_name} ({estimated_size / 1024:.1f}KB), 출처: {origin_url}")
        
        file_bytes = base64.b64decode(file_b64)
        extension = original_file_name.split('.')[-1].lower() if '.' in original_file_name else ""
        
        try:
            detected, _, _, _ = handle_input_raw(file_bytes, extension)
            
            final_detections = detected
...
async def handle_text_event(request: Request):
    ...
    try:
        ...
        for d in detected: 
            ...
            if t == "card":
                card_total += 1
                if st == "valid": card_valid += 1
                else: card_invalid += 1
            if t == "ssn":
                ssn_total += 1
                if st == "valid": ssn_valid += 1
                else: ssn_invalid += 1
...
async def handle_combined(request: Request):
    ...
    try:
        ...
        if text.strip():
            detected_text = detect_by_regex(text) + detect_by_ner(text)
            
            final_text_detections = detected_text
        ...
        for file_info in files_data:
            original_file_name = file_info.get("name", "unknown")
            ...
            if file_b64:
                file_bytes = base64.b64decode(file_b64)
                extension = original_file_name.split('.')[-1].lower() if '.' in original_file_name else ""
                detected_file, _, _, _ = handle_input_raw(file_bytes, extension)
                
                final_file_detections = detected_file
