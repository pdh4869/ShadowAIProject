# ===== app.py (최종 통합본: 메타데이터 DB 저장 + 파일 내용 디스크 저장 + CORS 적용) =====
...
try:
    # host.py의 gather_network_info 함수를 사용하기 위해 import 시도
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # 모든 출처 허용 (PoC/테스트용)
...
async def api_file_collect(req: FileCollectRequest):
    ...
    if req.kind == "file" and req.data_b64:
        try:
            ...
            # sys.stderr import는 native_host/host.py에 있지만,
            # FastAPI 환경에서는 print(e, file=sys.stderr) 대신 print(e)를 사용합니다.
