...
# Desc: SC 서버 버전에 파일명 마스킹(Logic.py)을 통합. 조합위험도/그룹 저장/대시보드 유지
# =============================
...
# 동일 디렉토리의 Logic_SC_Hybrid에서 import
...
def verify_auth(request: Request) -> bool:
    ...
    try:
        ...
        if abs(now_ms - req_time) > 300000:  # 5분
            print(f"[SECURITY] 타임스탬프 만료: {abs(now_ms-req_time)}ms")
            return False
    except:
        return False
...
async def handle_file_collect(request: Request):
    ...
    try:
        ...
        if est > HARD_LIMIT:
            return JSONResponse(content={"status":"에러","message":"파일이 너무 큽니다 (최대 100MB)"}, status_code=413)
        if est > SOFT_LIMIT:
            print(f"[WARN] 큰 파일 처리 중: {est/1024/1024:.1f}MB - {file_name}")

        print(f"[INFO] 파일 수신: {file_name} ({est/1024:.1f}KB), 출처: {origin_url}")

        # 파일명 마스킹 우선 적용 (Logic.py 방식)
        ...
        if detected:
            ...
            for it in detected:
                ...
                print(f"[INFO] ✓ 파일 탐지: {it.get('type')} = {it.get('value')}{st}")
...
if __name__ == "__main__":
    # 예: uvicorn LocalServer_SC_Hybrid:app --reload --port 7861
