...
# ★ 수정 완료: User-Agent 전용 렌더링 함수 (길이 제한 제거)
def render_ua_cell(js) -> str:
    ...
    try:
        ...
        # ★ 수정: 길이 제한 없이 전체 문자열 반환
        return ua
    except Exception:
        return "-"

# User-Agent 표시 로직이 제거된 OS 렌더링 함수 (유지)
...
async def admin():
    ...
    for r in rows:
        ...
        ua_cell = render_ua_cell(r["network_info"]) # 길이 제한 없이 전체 출력
        ...
        html.append(
            ...
            f"<td>{os_cell}</td>"      # OS만 표시
            f"<td>{ua_cell}</td>"      # User-Agent 전체 표시
            f"<td>{files_cell}</td>"
            f"</tr>"
        )
