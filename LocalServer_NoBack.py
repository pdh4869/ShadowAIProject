...
async def handle_file_collect(request: Request):
    ...
    try:
        ...
        try:
            ...
            if not detected:
                print("[INFO] 탐지 결과 없음 → 백엔드 전송 생략")
                backend_status = "no_detection"
            else:
                # === 백엔드 전송 로직 추가 ===
                try:
                    # 탐지 요약 통계 구성
                    ...
                    # ok = send_to_backend(json_data, filename=f"Detected_{file_name}.json", meta=meta_info)
                    # backend_status = "backend_success" if ok else "backend_failed"
                    backend_status = "backend_disabled" # 백엔드 전송 부활 시, 이 코드 지울 것
                    print(f"[INFO] 백엔드 전송 상태: {backend_status}")
                    logging.info("[DEBUG] 백엔드 전송 비활성화 (테스트 중)") # 백엔드 전송 부활 시, 이 코드 지울 것
                except Exception as e:
                    print(f"[ERROR] 파일 백엔드 전송 실패: {e}")
...
async def handle_text_event(request: Request):
    ...
    try:
        ...
        if not detected:
            print("[INFO] 탐지 결과 없음 → 백엔드 전송 생략")
            backend_status = "no_detection"
            return JSONResponse(content={"result": {"status": "탐지 결과 없음"}}, status_code=200)
        ...
        try:
            ...
            # backend_status = "backend_success" if ok else "backend_failed"
            backend_status = "backend_disabled" # 백엔드 전송 코드 부활 시, 이 코드 삭제
            print(f"[INFO] 백엔드 전송 상태: {backend_status}")
            logging.info("[DEBUG] 백엔드 전송 비활성화 (테스트 중)") # 백엔드 전송 코드 부활 시, 이 코드 삭제
        except Exception as e:
...
async def handle_combined(request: Request):
    ...
    try:
        ...
        if files_data:
            ...
            # 최종 백엔드 전송 방지 조건
            no_text_detection = (not text.strip()) or (not detect_by_regex(text) + detect_by_ner(text))
            no_file_detection = True
            ...
            if no_text_detection and no_file_detection:
                print("[INFO] 텍스트와 파일 모두 탐지 결과 없음 → 백엔드 전송 생략")
                backend_status = "no_detection"
                return JSONResponse(content={"result": {"status": "탐지 결과 없음"}}, status_code=200)
