// background.js (수정된 전체 코드)
const PORT_NAME = "pii_port";

const SERVER_EVENT_ENDPOINT = "http://127.0.0.1:9123/api/event";
const SERVER_FILE_ENDPOINT  = "http://127.0.0.1:9123/api/file_collect";
...
async function handlePayload(t, payload, senderUrl) {
  ...
  try {
    const nativeResp = await callNative("get_full_info", {}); // get_ip -> get_full_info
    if (nativeResp?.ok && nativeResp.data) {
      networkInfo = nativeResp.data;
      console.log(`[bg] ✓ 네트워크 정보 수집 완료:`, networkInfo);
    } else {
      console.warn(`[bg] Native Host 네트워크 정보 응답 없음`);
    }
  } catch (e) {
    console.warn(`[bg] Native Host 네트워크 정보 수집 실패:`, e.message);
  }
  ...
  if (t === "COMBINED_EVENT") {
    ...
    console.log(`[bg] 통합 전송: http://127.0.0.1:9123/api/combined`);

    fetch("http://127.0.0.1:9123/api/combined", options)
      .then(res => res.json())
