...
def analyze_combination_risk(detected_items, text):
    # 카테고리별 분류
    identifier_items = [...]  # 이름, 주민번호, 전화번호 등
    quasi_items = [...]       # 조직명, 주소, 직책
...
def parse_file(File_Bytes: bytes, File_Ext: str):
    if File_Ext == "txt":
        ...
        # 단락 + 테이블 텍스트 추출
        text = extract_paragraphs_and_tables(doc)
...
**A:** `soddokayo/klue-roberta-base-ner` 모델을 사용했습니다. 한국어 특화 모델로, 이름(PS), 조직명(ORG), 주소(LC) 등을 탐지합니다. 단, 오탐지 방지를 위해 2글자 이하, 숫자만 있는 경우 등은 필터링합니다.

### Q3. "조합 위험도 분석이 뭔가요?" ⭐ 핵심 질문
**A:** 단일 정보만으로는 개인을 특정하기 어렵지만, 여러 정보가 조합되면 특정 가능합니다.

**예시:**
- "홍길동" 단독 → 탐지 안 함 (흔한 이름)
- "홍길동 + 삼성전자" → 중위험 (수백 명)
- "홍길동 + 삼성전자 + 부장 + 서울 강남구" → 고위험 (1~2명 특정 가능)

이를 자동으로 분석하여 위험도를 표시합니다.
...
### Q5. "오탐지는 어떻게 방지하나요?"
**A:** 3가지 방법으로 오탐지를 방지합니다:
1. **검증 로직**: 주민번호 체크섬, 카드번호 Luhn 알고리즘, 운전면허 지역코드 검증
2. **필터링**: NER 결과 중 2글자 이하, 숫자만 있는 경우 제외
3. **조합 분석**: 단일 정보는 탐지 안 하고, 조합 위험도가 있을 때만 표시
...
1. "홍길동" 입력 → 탐지 안 됨 (단독 이름)
2. "홍길동 삼성전자" 입력 → 중위험 (이름+조직명)
3. "홍길동 삼성전자 부장 서울 강남구" 입력 → 고위험 (조합)
