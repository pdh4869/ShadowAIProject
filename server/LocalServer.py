...
async def dashboard():
    """대시보드 페이지"""
    # SyntaxWarning 방지를 위해 Raw String (r) 적용
    html_content = r"""
    <!DOCTYPE html>
    <html lang="ko">
    <head>
                ...
                try {
                    ...
                    if (data.detections && data.detections.length > 0) {
                        logContainer.innerHTML = data.detections.map(d => {
                            // IP 표시 로직: simple 'ip' key 또는 complex 'interfaces' key 확인 (1/2)
    ...
    """
    # Content-Length 오류 해결을 위해 Response 사용 및 UTF-8 인코딩 명시
    return Response(
