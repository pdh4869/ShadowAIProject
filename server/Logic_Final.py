...
# --- 주소 패턴 및 보조 탐지 함수 추가
# 시/도 표기(예: '서울시','부산시','경기도')를 허용하고 시/도와 구/동 사이의 공백 유무 모두 대응합니다.
# 추가: 도로명, 지번, 우편번호(5~6자리/3-3 형식), 영문 주소(예: '123 Baker St' 또는 '12-34 Yongsan-ro') 등을 보조로 탐지합니다.
...
# 우편번호 (국내 5자리 또는 3-2 형식, 과거 6자리도 일부 케이스로 허용)
ZIP_PATTERN = re.compile(r'(?<!\d)(?:\d{5}|\d{3}-\d{2}|\d{6})(?!\d)')
...
def detect_names_by_regex(text: str) -> list:
    """간단 보조: 한글 2~4자 단어를 이름 후보로 탐지(블랙리스트/성씨 필터 적용)."""
    out = []
    ...
    try:
        ...
        for entity in ner_results:
            ...
            if Label in ['PS', 'PER']:
                ...
                if len(clean_word) <= 1 or clean_word.isdigit() or clean_word in NAME_BLACKLIST or not any('\uac00' <= c <= '\ud7a3' for c in clean_word):
                    continue
...
def categorize_detection(item_type: str) -> str:
    ...
    if item_type in ['PS','PER', 'phone', 'email', 'ssn', 'alien_reg', 'driver_license', 'passport', 'card', 'account']:
        return 'identifier'
    if item_type in ['ORG','OG','student_id','birth','LC','position', 'ip', 'LOC', 'address']:
        return 'quasi'
    return 'other'
...
def parse_file(File_Bytes: bytes, File_Ext: str) -> tuple:
    ...
    if File_Ext == "txt":
        ...
        try:
            ...
            text = text.strip()
            # HWP 텍스트 정리 (태그만 제거, 내용은 보존) - Logic (1).py 스타일
            ...
            while True:
                prev = text
                # Logic (1).py의 정규식: r'([가-힣])\s+([가-힣])(?=\s|[가-힣]|$)'
...
def handle_input_raw(Input_Data: bytes, Original_Format: str = None, Original_Filename: str = None):
    ...
    if combined_text:
        ...
        # [수정] Logic은 모든 탐지 결과를 반환하도록 변경 (서버가 필터링)
        ...
        # [수정] 기존 else 필터링 로직 제거
        # else:
        #    Detected = [i for i in all_detected if categorize_detection(i.get('type')) == 'identifier']

    # [수정] 얼굴 탐지 결과를 메인 'Detected' 리스트에 추가
    total_faces = 0
    ...
    backend_status = True # Logic (1).py의 반환값 형식을 맞추기 위해 False -> True로 가정

    # Logic (1).py는 4개의 값을 반환 (Detected, masked_filename, backend_status, image_detections)
    # Logic_Final.py도 4개의 값을 반환하므로 형식이 동일합니다.
    # [수정] image_detections 대신 빈 리스트 반환 (Detected에 이미 포함됨)
