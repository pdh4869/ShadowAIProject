...
const PII_GROUPS = {
  ...
  '위치 정보': ['IP', '직책', '조직/기관', '주소'], '신분증/증명서': ['주민등록번호', '외국인등록번호', '운전면허번호', '여권번호'],
...
function openDetailModal(log) {
  ...
  // 탐지 정보
  $('#detail-source').textContent = log.file_type_name || log.source_type || '-';
  
  // 파일명
  const filenameSection = $('#filename-section');
  const isText = (log.file_type_name === '텍스트' || log.source_type === '텍스트');
  if (isText) {
