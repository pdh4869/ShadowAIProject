// personal_information_type.page.js - Flask 백엔드 데이터 사용 (수정)
...
// Flask에서 전달받은 실제 데이터 사용 (HTML에 추가된 변수명과 일치)
...
// 개인정보 유형 그룹 및 색상 (유지)
const PII_GROUPS = {
  '연락처 정보': ['전화번호', '이메일'], '신원 정보': ['이름', '생년월일'],
  '위치 정보': ['IP', '직책', '조직/기관'], '신분증/증명서': ['주민등록번호', '외국인등록번호', '운전면허번호', '여권번호'],
  '금융 정보': ['계좌번호', '카드번호'], '생체 정보': ['얼굴이미지'],
  '기타': ['조합위험도', '학번']
};
...
    const originalText = chip.dataset.piiType || chip.textContent.trim();
...
      
      // CSS 클래스 대신 JS에서 직접 너비 조정은 권장되지 않으나, 기존 코드 유지
      if (screenWidth < 768) {
        if (index === 0) th.style.width = '60px';
        else if (index === 1) th.style.width = '80px';
        else if (index === 2) th.style.width = '70px';
        else if (index === 3) th.style.width = '120px';
        else if (index === 4) th.style.width = '70px';
        else if (index === 5) th.style.width = '80px';
...
function openDetailModal(log) {
  // log 객체 속성은 Flask의 `logs` 리스트 구조에 맞춥니다.
...
  const filenameSection = $('#filename-section');
  const isText = log.file_type_name === '텍스트';
  if (isText) {
...
  const piiNames = log.pii_types_names || [];
  if (piiNames.length > 0) {
    typesEl.innerHTML = piiNames.map(type => {
      const koreanType = translatePIIType(type);
...
  }
  ...
  // 검증 결과 (HTML에 validation-section이 있으므로, 데이터가 없어도 구조 유지)
  $('#validation-section').parentElement.style.display = 'none'; // 데이터 없으므로 기본 숨김 처리
  $('#detail-validation').textContent = '-'; 
  
  // 개인 식별 의심 (데이터에 'suspicious' 키가 없으므로 임시로 '-' 처리)
  $('#detail-suspicious').textContent = '-';
...
function renderChart() {
  // CHART_DATA는 PII 유형별 분포 데이터({type: 'ssn', count: 10})를 예상합니다.
...
        tooltip: { callbacks: { label: (context) => `${context.label}: ${context.parsed.y}건` } }
      },
      onClick: (event, elements) => {
        if (elements.length > 0) {
          const index = elements[0].index;
          const selectedType = CHART_DATA[index].type;
          
          // PII 유형 필터링을 위해 드롭다운 값 변경 후 서버 재요청
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (fromValue && toValue) {
      ...
      $('#to').value = toValue;
      $('#quickDesc').textContent = `· 선택 기간: ${fromValue} ~ ${toValue}`;
  } else {
    setTodayDate();
