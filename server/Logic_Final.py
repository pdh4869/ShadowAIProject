# =============================
# File: Logic_Final.py
# Desc: 10.15 ver/server/Logic_Final.py 버전에
#       Logic (1).py의 고유 기능 (HWP 띄어쓰기 정규화, 이름 화이트리스트)을
#       통합하고, handle_input_raw가 모든 탐지 결과를 반환하도록 수정한
#       최종 완성본입니다.
#
# [수정] 2025-10-22 (오탐지 개선 v2):
# 1. PS(인명) 오탐지 원인인 보조 함수(detect_names_by_regex) 비활성화
#    (사용자 요청: 블랙리스트 방식이 아닌 "이름 로직 강화" -> NER 모델 전담)
# 2. zipcode(우편번호) 오탐지를 줄이기 위해 6자리(구형) 패턴 제거
#
# [수정] 2025-10-22 (사용자 로그 기반 수정):
# 1. HWP -> HWPX 변환 시 COM 오류 (-2147352562) 방지를 위해 절대 경로 사용 (os.path.abspath)
# 2. HWP(olefile) 파싱 시, 본문 텍스트와 이미지 OCR 결과를 항상 병합하도록 로직 수정
# =============================
...
try:
    ...
    Presentation = None
try:
    import win32com.client as win32com # [수정] win32com.client as win32com으로 변경
except ImportError:
    win32com = None
try:
...
# (detect_names_by_regex가 비활성화되어 아래 리스트는 NER 내부 로직에서만 사용됨)
...
# --- [추가된 기능] Logic (1).py의 이름 화이트리스트 ---
NAME_WHITELIST = {'홍길동'}
# --- [추가 완료] ---

# --- 주소 패턴 및 보조 탐지 함수 추가
ADDR_PATTERN = re.compile(r'(?:서울(?:특별시|시)?|부산(?:광역시|시)?|대구(?:광역시|시)?|인천(?:광역시|시)?|광주(?:광역시|시)?|대전(?:광역시|시)?|울산(?:광역시|시)?|경기(?:도)?|경남(?:도)?|경북(?:도)?|전남(?:도)?|전북(?:도)?|충남(?:도)?|충북(?:도)?|세종(?:특별자치시)?|제주(?:특별자치도)?)[\s]*(?:[가-힣]+(?:시|군|구))[\s]*(?:[가-힣가-힣]+(?:읍|면|동|가|로|길))?(?:[\s]+[0-9-]+)?')

# [수정] 6자리 우편번호(구형) 탐지 로직이 운전면허번호 등과 충돌하여 5자리(신형)만 탐지하도록 변경
ZIP_PATTERN = re.compile(r'(?<!\d)(?:\d{5}|\d{3}-\d{2})(?!\d)')

# 도로명/지번 (간단한 보조 정규식)
STREET_PATTERN = re.compile(r'(?:(?:[0-9]{1,5}[-–‑]?[0-9]{0,5})\s*(?:[가-힣]+로|[가-힣]+길|[가-힣]+대로|[가-힣]+로\d*|[가-힣]+동)\b)')

# 영문 주소 (숫자 + 거리명 또는 도로명 영어표기 포함)
EN_ADDR_PATTERN = re.compile(r'(?<!\w)(?:\d{1,5}\s+(?:[A-Za-z0-9\-]+\s){0,4}(?:Street|St\.?|Avenue|Ave\.?|Road|Rd\.?|Boulevard|Blvd\.?|Lane|Ln\.?|Drive|Dr\.?|Court|Ct\.?|Square|Sq\.?|Place|Pl\.?|Terrace|Ter\.?|Way)\b)', re.IGNORECASE)

# [수정] PS(인명) 오탐지를 유발하는 보조 함수 비활성화
def detect_names_by_regex(text: str) -> list:
    """
    [비활성화] 간단 보조: 한글 2~4자 단어를 이름 후보로 탐지(블랙리스트/성씨 필터 적용).
    (오탐지율이 높아 NER 모델 전담으로 변경)
    """
    return []
    
    # --- 아래 로직은 비활성화 ---
    # out = []
    # for m in re.finditer(r'(?<![가-힣])[가-힣]{2,4}(?![가-힣])', text):
    #     name = m.group()
    #     if name in NAME_BLACKLIST:
    #         continue
    #     if name[0] not in KOREAN_SURNAMES:
    #         continue
    #     out.append({"type": "PS", "value": name, "span": m.span()})
    # return out
...
def detect_by_ner(Text: str) -> list:
    ...
    try:
        ...
        for match in name_patterns.finditer(Text):
            ...
            if name and name[0] in KOREAN_SURNAMES:
                if name not in NAME_BLACKLIST:
                    Detected.append({"type": "PS", "value": name, "span": match.span(1), "score": 1.0})
        ...
        for whitelist_name in NAME_WHITELIST:
            if whitelist_name in Text:
                is_duplicate = False
                for d in Detected:
                    if d['value'] == whitelist_name:
                        is_duplicate = True
                        break
                if not is_duplicate:
                    try:
                        for m in re.finditer(re.escape(whitelist_name), Text):
                            Detected.append({"type": "PS", "value": whitelist_name, "span": m.span(), "score": 1.0})
                            logging.info(f"[INFO] ✓ 화이트리스트 이름 탐지 (PS): {whitelist_name}")
                    except Exception:
                        pass
        # --- [추가 완료] ---
        ...
        for entity in ner_results:
            ...
            if Label == 'LC':
                addr_keywords = ['시', '군', '구', '동', '읍', '면', '로', '길']
                if any(kw in Word for kw in addr_keywords):
                    Detected.append({
                        "type": "LC",
                        "value": Word,
                        "span": (Start, End),
            ...
            if Label in ['PS', 'PER', 'ORG', 'OG', 'LOC']:
                if Label in ['ORG', 'OG'] and Word in NAME_BLACKLIST:
                    continue
    ...
    if location_parts:
        ...
        Detected.append({"type": "LC", "value": ' '.join(uniq), "span": (0,0)})
    
    deduplicated_final = []
    sorted_output = sorted(Detected, key=lambda x: x['span'][0])
    last_end = -1
...
def detect_quasi_identifiers(text: str) -> list:
    ...
    for m in position_pattern.finditer(text):
        if m.group() not in NAME_BLACKLIST:
...
def categorize_detection(item_type: str) -> str:
    if item_type in ['image_face']:
        return 'sensitive'
    if item_type in ['PS','PER', 'phone', 'email', 'ssn', 'alien_reg', 'driver_license', 'passport', 'card', 'account']:
        return 'identifier'
    # [수정] zipcode 추가
    if item_type in ['ORG','OG','student_id','birth','LC','position', 'ip', 'LOC', 'address', 'zipcode']:
        return 'quasi'
    return 'other'
...
def analyze_combination_risk(detected_items, text):
    ...
    id_types = sorted(list(set([_translate_type(i['type']) for i in identifiers])))
    q_types = sorted(list(set([_translate_type(i['type']) for i in quasis])))
    sen_types = sorted(list(set([_translate_type(i['type']) for i in sensitives])))

    risk_level = None
    risk_msg = None
    risk_items = []
    
    id_str = f"식별자({','.join(id_types)}){id_cnt}건" if id_cnt > 0 else ""
    q_str = f"준식별자({','.join(q_types)}){q_cnt}건" if q_cnt > 0 else ""
    sen_str = f"민감정보({','.join(sen_types)}){sen_cnt}건" if sen_cnt > 0 else ""

    if sen_cnt > 0 and id_cnt > 0:
        risk_level = 'critical'
        risk_msg = f"{sen_str}+{id_str} → 개인 완전 특정 가능"
        risk_items = sensitives + identifiers
    elif id_cnt >= 1 and q_cnt >= 2:
        risk_level = 'high'
        risk_msg = f"{id_str}+{q_str} → 개인 특정 가능"
        risk_items = identifiers + quasis
    elif id_cnt >= 2:
        risk_level = 'high'
        risk_msg = f"{id_str} → 개인 특정 가능"
        risk_items = identifiers
    elif id_cnt >= 1 and q_cnt >= 1:
        risk_level = 'medium'
        risk_msg = f"{id_str}+{q_str} → 개인 특정 가능성"
        risk_items = identifiers + quasis
    elif q_cnt >= 3:
        risk_level = 'medium'
        risk_msg = f"{q_str} → 개인 특정 가능성"
        risk_items = quasis
    ...
    if risk_level:
        return {
            ...
            'message': risk_msg,
            'items': risk_items,
            'counts': {'identifier': id_cnt, 'quasi': q_cnt, 'sensitive': sen_cnt}
        }
    return None
    ...
    if File_Ext == "txt":
        ...
        if win32com is not None:
            try:
                ...
                hwp.SaveAs(os.path.abspath(hwpx_path), "HWPX"); hwp.Quit()
        ...
        try:
            # [FIX 2] HWP(ole) 파싱 시, 본문 텍스트와 이미지 OCR 텍스트를 항상 병합하도록 수정
            
            ole = olefile.OleFileIO(io.BytesIO(File_Bytes))
            text = "" # 본문 텍스트
            ocr_text = "" # 이미지 OCR 텍스트
        ...
        if xlrd is None:
            ...
            try:
                ...
                wb = excel.Workbooks.Open(os.path.abspath(tmp_path)) # [FIX 1] 여기도 절대경로 적용
                wb.SaveAs(os.path.abspath(xlsx_path), FileFormat=51); wb.Close(SaveChanges=False); excel.Quit() # [FIX 1] 여기도 절대경로 적용
        ...
        if File_Ext == "ppt":
            ...
            try:
                ...
                pres = pp.Presentations.Open(os.path.abspath(tmp_path), WithWindow=False) # [FIX 1] 여기도 절대경로 적용
                pres.SaveAs(os.path.abspath(pptx_path), FileFormat=24); pres.Close(); pp.Quit() # [FIX 1] 여기도 절대경로 적용
...
def handle_input_raw(Input_Data: bytes, Original_Format: str = None, Original_Filename: str = None):
    ...
    if combined_text:
        ...
        # [수정] detect_names_by_regex() 호출은 유지하되, 함수 내부가 비활성화됨
        name_results  = detect_names_by_regex(combined_text) 
        
        address_results = detect_address(combined_text)

        all_detected, seen = [], set()
        for it in (regex_results + ner_results + quasi_results + name_results + address_results):
    ...
    return Detected, (masked_filename or ""), backend_status, []