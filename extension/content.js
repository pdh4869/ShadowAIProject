// content.js – 최종 버전: 프롬프트 영역에서만 탐지되도록 수정됨
(function () {
  ...
  document.addEventListener("keydown", async (e)=>{
    if (e.key==="Enter" && !e.shiftKey && !e.ctrlKey && !e.altKey && !e.metaKey) {
      ...
      // 프롬프트 입력창으로 간주할 수 있는 요소인지 확인 (textarea, contenteditable, role="textbox")
      const isPromptArea = activeEl && (
        activeEl.tagName?.toLowerCase() === 'textarea' ||
        activeEl.getAttribute('role')?.toLowerCase() === 'textbox' ||
        activeEl.isContentEditable
      );

      if (isPromptArea) {
        console.log("[content] ✓ Enter 키 전송 (프롬프트 영역)");
        ...
        console.log("[content] Enter 키 눌렸으나, 프롬프트 영역이 아님");
      }
    }
  }, true);

  // 클릭 이벤트 로직 수정을 위해 mousedown에서 텍스트 미리 캡처
  ...
  document.addEventListener("mousedown", (e)=>{
    ...
    if (isPromptArea) {
        lastCapturedText = getFocusedText().trim(); // 프롬프트 영역일 때만 텍스트 저장
        console.log(`[content] mousedown - 프롬프트 텍스트 캡처: ${lastCapturedText.length}글자`);
    } else {
        lastCapturedText = ""; // 프롬프트 영역이 아니면 텍스트 초기화
    }
  }, true);
  ...
  document.addEventListener("click", async (e)=>{
    ...
    for (let i=0;i<8&&el;i++,el=el.parentElement){
      ...
      // 전송 버튼이 클릭되었고, 이전에 프롬프트 영역에서 캡처된 텍스트가 있을 때만 전송
      if (isSendButton && (pendingFiles.length > 0 || lastCapturedText)) {
        isSending = true;
        console.log(`[content] 전송 버튼 클릭: 텍스트 ${lastCapturedText.length}글자, 파일 ${pendingFiles.length}개`);
