...
def categorize_detection(item_type: str) -> str:
    if item_type in ['image_face']:
        return 'sensitive'
    if item_type in ['PS','PER', 'phone', 'email', 'ssn', 'alien_reg', 'driver_license', 'passport', 'card', 'account', 'ip']:
        return 'identifier'
    if item_type in ['ORG','OG','student_id','birth','LC','position', 'LOC']:
        return 'quasi'
    return 'other'
...
def analyze_combination_risk(detected_items, text):
    ...
    if sen_cnt > 0 and id_cnt > 0:
        ...
        # 준식별자도 포함
        if q_cnt > 0:
            risk_msg = f"{sen_str}+{id_str}+{q_str} → 개인 완전 특정 가능"
            risk_items = sensitives + identifiers + quasis
        else:
