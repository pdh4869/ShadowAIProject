// pdh4869/shadowaiproject/ShadowAIProject-agent-InfoHooking-/네트워크 탐지/extension/background.js - 수정 완료
...
async function forwardToServerFromContent(contentPayload, sender) {
  ...
  const finalPayload = {
    ...
    // 필요시 추가 전달 필드
    llm: norm.llm,
    tab: norm.tab             // ★ 추가: 브라우저 탭 정보 (user-agent 포함)
  };
...
// ★ 추가: 허용된 LLM 도메인 목록 및 검사 함수 (ChatGPT 및 Gemini만)
...
chrome.runtime.onMessage.addListener((msg, sender) => {
    ...
    const url = sender?.tab?.url || sender?.url || msg.payload?.source_url;

    // ★ 수정: URL 검사 로직 추가 (ChatGPT/Gemini만 통과)
    if (!isAllowedLLMUrl(url)) {
        console.log("[bg] event blocked: not an allowed LLM host:", url);
        return; 
    }

    console.log("[bg] event from content (LLM detected):", url);
